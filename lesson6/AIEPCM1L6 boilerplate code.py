...
def load_data(file_path=r"c:/Users/Public/AI expert/lesson6/imdb_top_1000.csv"):
    try:
...
def recommend_movies(genre=None, mood=None, rating=None,top_n=5):
    ...
    for idx,row in filtered_df.iterrows():
        ...
        if (mood and ((TextBlob(mood).sentiment.polarity<0 and polarity>0)or polarity>=0))or not mood:
            recommenditions.append((row['Series_Title'], polarity))
        if len(recommenditions)==top_n:
            break
    return recommenditions if recommenditions else 'no suitable movie found'


# Display recommendations🍿 😊  😞  🎥
def display_recommendations(recs,name):
    print(Fore.CYAN+f"\n{name}, here are your recommendations:\n")
    for idx,(title,polarity) in enumerate(recs,1):
        sentiment="positive" if polarity>0 else "negative" if polarity<0 else "netural"
        print(Fore.GREEN+f"{idx}. {title} - mood: {sentiment}")
