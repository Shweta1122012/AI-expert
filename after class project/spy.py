...
while True:
    ...
    if sentiment == 'positive':
        follow_up = input(f"{Fore.CYAN}That's great! What made you feel so positive? {Style.RESET_ALL}")
    elif sentiment == 'negative':
        follow_up = input(f"{Fore.CYAN}I'm sorry you feel that way. Would you like to talk about it? {Style.RESET_ALL}")
    else:
        follow_up = input(f"{Fore.CYAN}Interesting neutral response. Could you elaborate more? {Style.RESET_ALL}")
    
    
    if follow_up.strip():
        follow_polarity = TextBlob(follow_up).sentiment.polarity
        conversation_history.append((follow_up, follow_polarity, 
            'positive' if follow_polarity > 0.25 else 'negative' if follow_polarity < -0.25 else 'neutral'))
